...
import { BookingStatus, ServiceType, BookingCategory } from '../../backend';
import {
  Table,
  TableBody,
  TableCell,
  ...
  if (bookings.length === 0) {
    return (
      <div className="card-cosmic rounded-xl p-16 text-center">
        <p className="font-cinzel text-sm tracking-widest text-gold/40 uppercase mb-2">No Bookings Yet</p>
          ...
          <div key={String(booking.id)} className="card-cosmic rounded-xl p-5 space-y-3">
            ...
            <div className="space-y-2">
              <div className="flex items-start gap-2">
                ...
                <div>
                  <p className="font-inter text-sm text-foreground/80 font-medium">{booking.customerName}</p>
                  <p className="font-inter text-xs text-foreground/50">
                    {formatServiceType(booking.serviceType)} · {formatCategory(booking.category)}
                  </p>
                </div>
              </div>
              <div className="flex items-center gap-2">
                <Phone className="w-3.5 h-3.5 text-gold/50 shrink-0" />
                <p className="font-inter text-sm text-foreground/70">{booking.phoneNumber}</p>
              </div>
              <div className="flex items-center gap-2">
              ...
              {booking.message && (
                <div className="flex items-start gap-2">
                  ...
                  <p className="font-inter text-sm text-foreground/60 italic">{booking.message}</p>
                </div>
              )}
            </div>
          </div>

...
export default function AdminAnalytics({ bookings }: AdminAnalyticsProps) {
  ...
  for (const s of Object.values(ServiceType)) {
    serviceCounts[s] = 0;
  }
  ...
  const confirmedCount = bookings.filter(
    (b) => (b.status as unknown as string) === BookingStatus.confirmed,
  ).length;
  ...
  return (
    <div className="mb-8 rounded-2xl border border-gold-400/20 bg-cosmic-900/60 overflow-hidden">
      <div className="px-6 py-4 border-b border-gold-400/10 bg-gradient-to-r from-cosmic-900 to-cosmic-800">
        <h2 className="font-cinzel text-lg font-bold text-gold-400">
          📊 Booking Analytics
        </h2>
      </div>
      ...
      <div className="p-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
          <div className="rounded-xl border border-gold-400/20 bg-cosmic-800/50 p-4 text-center">
            <div className="text-3xl font-cinzel font-bold text-gold-400">
              {total}
            </div>
            <div className="text-cosmic-400 text-xs font-cinzel mt-1 uppercase tracking-wide">
              Total Bookings
            </div>
          </div>
        ...
        <div>
          <h3 className="font-cinzel text-sm text-cosmic-300 uppercase tracking-widest mb-4">
            Service Breakdown
          </h3>
